...
DropdownConst.prototype.appendData = function(name,appentoWhat){
	//console.log(name);
	var epn = document.getElementById(appentoWhat) || false;
	epn.innerHTML="";
...
function getVpcName(){
	var dt = document.getElementById("dtN"),
		...
		$.getJSON(_ip+'/deploydbData', function(d , responce){
...
var ddT = {
	...
	clc: function(){
		...
		$.ajax({
			     ...
			     success: function(results){
					console.log(results)
					results == "saved" ? alert("Inserted values sucessfully") (location.href=_ip+"/viewDevOpsTemplate") : false;
				 },
				 error: function (xhr, status, error){
...
function getSubnetName(vpcid){
	...
	var subNetName1 = [];
	$(function(){
		  $.getJSON('http://172.29.59.62:3000/subnet_deploy', function(data){
			  console.log(data);
...
$('.exit').click(function(){
	location.href = _ip+"/viewDevOpsTemplate";
})
